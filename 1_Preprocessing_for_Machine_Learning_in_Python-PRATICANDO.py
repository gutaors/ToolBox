...
# ### Inspeção
# ### Qual o comando pra inspecionar?
...
# ### Dados Faltantes, Missing data - rows
# ### Tem comando para ver faltantes?
# tem e tem outro pra criar dataset só com não faltantes
...
#is_priority_subset = volunteer[volunteer['is_priority'].notnull()]

# Print out the shape of the subset
#print(is_priority_subset.shape)

#is_priority_subset.head()


# ### Convertendo tipo da coluna
# Qual o comando?
# ### Exibindo tipos dos dados
# como exibe?
# 
...
#removendo dólar e vírgula e depois convertendo para float ( REPLACE E CONVERT )
discoveries['value'] = discoveries['value'].replace({'\$': '', ',': ''}, regex=True)
discoveries['value'] =discoveries['value'].astype(float)
...
### como mostrar os campos com nulls, lista ordenada do que tem menos para o que tem mais
...
# Lista os indices (tdos, os que tem nulls e os que não tem). 
# Observe que são os mesmos nomes acima, só que em uma listinha separada por vírgula) 
...
# Cria uma lista com os nomes listados acima, esta lista é nomeada de volunteer
...
# usa a lista volunteer para gerar volunteer_X e volunteer_y

from sklearn.model_selection import train_test_split

# Cria dadaset com todas as colunas exceto category_desc
volunteer_X = volunteer.drop("category_desc", axis=1)

# Cria um dataset (y) somente com o label category_desc (y é o alvo, embora meu cérebro queira que seja X)
volunteer_y = volunteer[["category_desc"]]

# Usa amostragem estratificada pra dividir o dataset de acordo com o dataset volunteer_y
X_train, X_test, y_train, y_test = train_test_split(volunteer_X, volunteer_y, stratify=volunteer_y)

# Exibe as contagems de  category_desc nos labels de treinamento y
...
# ### lê o csv de vinhos, um csv interessante hein, em seguida faz um X e um y
...
# ### Ele vai rodar um modelo de KNN para rodar um score.
# ### Este score a gente vai comparar com outro depois de normalizar 

# In[57]:


# faz o split em X e y e depois roda KNN
...
# observe a variância, ela é importante, está em 99166.71
# já a variância do log é 0.17, que diferença hein
...
# ### KNN no dado que não foi scaled (não mudou de escala)
...
# Criamos o Scaler com StandardScaler
ss = StandardScaler()

# Pega um subset do DataFrame que você quer fazer scale com algumas colunas
wine_subset = wine[['Ash', 'Alcalinity of ash', 'Magnesium']]

# Aplica o  scaler ao novo subset do DataFrame que se chama wine_subset
...
# remove do dataset original as três colunas que fizemos subset
wine = wine.drop(['Ash', 'Alcalinity of ash', 'Magnesium'], axis = 1)
# concatena no DATAFRAME original o SUBSET com as três colunas que fizemos scale (aplicamos StandardScaler)
...
# ## Feature Engineering
# ## A Engenharia das Features não tem a ver com Scale, tem mais a ver com encode
# ### e quando falo de encode, lembro do onehotencode
