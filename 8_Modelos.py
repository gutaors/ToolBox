...
# carrega csv
# inpeciona 
# converte colunas texto em números
# 
# faz um pipeline-----------------------------------------------
# 
# separa em X e y
...
# inspeciona resultado com get_support()
# roda grid search para achar a max_depth
# pega o valor encontrado pela grid_search e roda uma random
# roda um selectKBest com chi2 para pegar as 100 features do topo
# cria novo dataset reduzido com features selecionadas
# 
# Human in the loop-------------------------------------------------------
# le csv
# faz head
# cria função featurize para pegar len de alguns campos e mean de outros
# cria uma lista de bads, com alguns códigos de computadores (o dataset é de computadores sofrendo ataques)
# roda cross_val_score
# agrupa por computador e aplica featurize
# converte iterador em dataframe chamando comando list
# verivica quais X.index estão na lista bads (ruins de criar labels)
# imprime media do cross do adaboost 
# 
# Ele agrupou as features em uma linguiça, agora vamos aprender a fazer feature engineering nestes dados agrupados
# 
# cria contagem de features com protocolo único por fonte
# converte para dataframe nomeando a coluna
# concatena com o dataset anterior
# refit o classificador e reporta sua acurácia
# 
# transformando heuristica em classificador
# cria novo dataset bad selecionando hosts da lista bad
# calcula méida de partes únicas nos exemplos bad
# poe label positivo nos sources que usam mais portas que aquilo
# imprime acurácia
# 
# combinando heurísticas
# computa média do pacote médio dos bad sources
# põe label de positivo se pacote médio é menor que aquele 
# encontra indices com pred_port e pred_packet são ambos verdadeiros
# imprime acurácia e compara com o ports
# 
# Lidando com ruido no label
# fit um naive bayes classifier ao dado de treinamento
# reporta acurácia nos dados de teste
# atribui metade do peso aos primeiros 100 exemplos ruidosos (noisy)
# refit usando os pesos e acurácia reportados, melhorou?
# 
# 
# Lembrete das métricas de performance
# printa os score e precision
# 
# análise de custos do mundo real
# fita uam random classifier
# faz threshold
# otimiza threshold (limite)
# junta tudo
# 
# Administração de ciclo de vida de modelo
# primeiro pipeline
# custom scores
# pickes - usa um cara chamado pickle
# Custom function transformers in pipelines¶
# Challenge the champion
# Cross-validation statistics¶
# Tuning the window size¶
# junta tudo
# workflows não superviosionados
# novidade silmpes
# tres detectores de novidades
# contaminação
# encontre o vizinho
# nem todas as métricas concordam
# Levensthein restrita
# juntando tudo

# ## Workflow padrão, como começa? 
# 

# #### importa as bibliotecas principais nupapy Nu Papai
# 
# coloca matplot inline? pra que serve isto? porque só ele?
# 
...
# Lê o credit.csv joga no dataset chamado credit
# faz um head pra olhar
...
# faz uma lista com colunas não numericas chamada non_numeric_columns
...
non_numeric_columns = ( 'checking_status', duration'credit_history	purpose	credit_amount	savings_status	employment	installment_commitment	personal_status	other_parties	...	property_magnitude	age	other_payment_plans	housing	existing_credits	job	num_dependents	own_telephone	foreign_worker	clas )


# In[47]:


# faz uma listinha com colunas não numéricas, pelo que entendi descobriu as colunas por inspeção visual
...
# imprime os tipos das colunas do dataset credit
...
print(credit.dtypes)
#vamos remover estas que estão como object


# IMPORTA LABELENCODER E RODA NAS COLUNAS NÃO NUMÉRICAS DO DATASET
...
# o que faz o labelencoder do sklearn:??
# ele faz um for nas colunas não numéricas e roda um fit_transform
...
# ### Your first pipeline (trata e roda um randomforest no dataset credit)

# separa o x e y, olha lá em cima os nomes das features e vai jogando no X os nomes certos, o y é só um né, o alvo
...
# importa os três mais importantes, train_test_split, RandomForestClassifier e accuracy_score
# ou seja, divide, classifica e mede acurácia
...
#Separa em treino e teste (train_test_split)
#importa os 3 mosqueteiros (train_test_split, RandomForestClassifier e accuracy_score
# roda uma Floresta Aleatória e mede a acurácia

from sklearn.model_selection import train_test_split
from sklearn.ensemble import RandomForestClassifier
from sklearn.metrics import accuracy_score

# Separa os dados em traino e teste, com 20% para teste (treina muito e testa pouco até em prova de concurso é assim né)
...
# ravel retorna ndarray com valores flat
#  o values fala que quer um array e não um vetor (coluna) e o ravel achata este cara pra 1d 
#  (flatten) que é o que o fit espera receber
...
# ### Grid search CV for model complexity (pesquisa em grade)

# Agora vamos rodar o gridsearch, ele faz os cross validation de vários modelos com hiperparâmetros diferentes de uma vez
...
# Vai usar o GridsearchCV para três modelos, o randomforest, adaboost e knn classifier
# Interessante notar passamos diferentes conjuntos de parâmetros para o grid dependendo do algoritmo que ele vai rodar

# In[52]:


# Vamos rodar o GridSearchCV no RandomForest, passando param_grid que sao os parâmetros acima, 
# estima de 10 a 40 em passos de 10
...
#aqui é outro grid, para rodar um AdaBoostClassifier fiz até o teste de rodar com nome de grid2 =... e funcionou
...
# Aqui o GridSearchCV roda um KNeighborsClassifier
...
#print(pd.Series(credit_history_num))


# In[57]:


#resumindo aqui embaixo, codifica numericamente o campo  credit_history, cria nova matriz feature com dummies 
# para este cara (credit history), compara o número de features do novo dataframe

# Cria codificação numérica para credit_history (histórico de crédito)
credit_history_num = LabelEncoder().fit_transform(
  credit['credit_history'])

# Cria uma nova matriz de features, incluindo codificação numérica (numeric encoding)
...
# Compare the number of features of the resulting DataFrames o novo dataframe tem 25 campos o anterior 21
...
# Função que computa diferença absoluta da média da coluna
def abs_diff(x):
    return np.abs(x-np.mean(x))

# Aplique-o ao valor do crédito e armazene-o em uma nova coluna
credit['diff'] = abs_diff(credit['credit_amount'])

# Crie um seletor de recurso com chi2 que escolha um recurso
sk = SelectKBest(chi2, k=1)

# Use o seletor para escolher entre credit_amount e diff
...
# ### Bringing it all together (aqui ele pega tudo, acha a profundidade ideal (max_depth), roda uma random forest e usa selectKbest para fazer um novo dataset com as features que importam)
...
# ### Is the source or the destination bad? (observe que aqui temos outro dataset, outras coisas)
...
#quando chamarmos esta função ela vai agrupar dados do mesmo computador, porta vai pegar o len 
# (quantas portas daquele computador sofreram ataques) pacote médio vai pegar a média da contagem de pacotes
# e duração média dos ataques
...
# Ajuste (FIT) um classificador Gaussian Naive Bayes aos dados de treinamento
...
# Dá erro de score, o que significa o seguinte 
#As mentioned in the comments, some labels in y_true don't appear in y_pred. 
#Specifically in this case, label '2' is never predicted:
# ou seja, warning só aparece na primeira vez que roda o código e significa que para alguns labels não houve previsão
# em outras palavras, ele não conseguiu calcular o y_pred para alguns labels, então o  
# set(y_test) - set(y_pred) não pode ser calculado e ele vai colocar um 0.0 para sinalizar isto
...
O cara tem uma porrada de número e a classe é 0 e 1, vamos rodar um classificador.
# In[110]:


from sklearn.tree import DecisionTreeClassifier

# # Crie um Pontuador (SCORER) atribuindo MAIS custo a falsos positivos
...
# Refite, reduzindo peso de quem está com peso abaixo de 80
...
#aqui abaixo a mágica acontece, ele pega o ytest, que é o y da base de teste,
# depois pega o X_test, que são as features da base de teste
# depois ele dá peso neste X e roda o clf_weighted nele, ou seja, uma Árvore de Decisão 
# com peso diferenciado para cada registro
# daí roda o my_scorer nisto, ou seja, passa os caras 
